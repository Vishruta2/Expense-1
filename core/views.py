    ...
    with transaction.atomic():
        with connection.cursor() as cur:
            ...
            for it in travel_fare:
                ...
                mode = it.get("mode_transport") or it.get("mode_transport") or ""
...
def api_auth_me(request):
    ...
    Response JSON:
      { name: str, designation: str, role: str, approve_seq: int, is_admin: bool }
    """
    auth = request.session.get("auth") or {}
    name = request.session.get("display_name") or auth.get("username") or ""
    role = request.session.get("role") or auth.get("role") or request.session.get("designation") or ""
    designation = role
    ...
    is_admin = "admin" in str(role or "").strip().lower()
    return JsonResponse({
        "name": name,
...
def api_projects_upsert(request):
    ...
    try:
        with connection.cursor() as cur:
            # Support saving edits as a NEW entry when requested
            save_as_new = str(data.get("save_as_new") or "").strip().lower() in ("1", "true", "yes")

            # Insert a NEW entry always when saving (per requirement)
    ...
    if ctype.startswith("application/json"):
        ...
        for it in base_fare:
            ...
            travel_fare.append({
                ...
                'mode_transport': it.get('mode_transport') or it.get('mode_transport') or it.get('mode') or '',
            ...
            if isinstance(travel_fare, list) and travel_fare:
                with connection.cursor() as cur:
                    for it in travel_fare:
                        ...
                        if not arr_t:
                            arr_t = '00:00:00'
                        mode   = it.get("mode_transport") or it.get("mode_transport") or it.get("mode") or ""
...
def api_validate_details(request, voucher_id: str):
    ...
    try:
        ...
        with connection.cursor() as cur:
            ...
            vtype, wo, total, empname, exp_date, upload_date = row
            details = {"voucher_no": voucher_id, "voucher_type": vtype, "work_order": wo,
                       ...
                       "upload_date": (upload_date.isoformat() if hasattr(upload_date, 'isoformat') else (str(upload_date) if upload_date is not None else ""))}
            ...
            if wo:
                try:
                    with connection.cursor() as cur2:
                        cur2.execute(
                            """
                            SELECT project_id, project_name, client_name, project_type, place, address, dept_id, status
                              FROM projects
                             WHERE work_order_no=%s
                             LIMIT 1
                            """,
                        ...
                        if prow:
                            details["project"] = {
                                "project_id": int(prow[0]) if prow[0] is not None else None,
                                "project_name": prow[1] or "",
                                "client_name": prow[2] or "",
                                "project_type": prow[3] or "",
                                "place": prow[4] or "",
                                "address": prow[5] or "",
                                "dept_id": int(prow[6]) if (prow[6] is not None and str(prow[6]).isdigit()) else None,
                                "status": prow[7] or "",
                            }
                except Exception:
                    pass
            ...
            if vtype == "PURCHASE":
                ...
                if travel_id:
                    # header fields from travel
                    cur.execute(
                        """
                        SELECT projectname, place, purpose_journey, place_visit,
                    ...
                    h = cur.fetchone()
                    if h:
                        # emit a simple travel header snapshot too
                        details.setdefault("travel_header", {})
                        details["travel_header"] = {
                            "projectname": h[0],
                            "place": h[1],
                            "purpose_journey": h[2],
                            "place_visit": h[3],
                            "from_date": (h[4].isoformat() if hasattr(h[4], 'isoformat') else (str(h[4]) if h[4] is not None else "")),
                            "to_date": (h[5].isoformat() if hasattr(h[5], 'isoformat') else (str(h[5]) if h[5] is not None else "")),
                            "visit_authorised": h[6],
                        }
                    # Detailed rows + sums
                    # Note: DB column is 'mode_transport' (sic). Also include dep/arr date/time.
                    cur.execute(
                        """
                        SELECT travel_fare_id, from_place, to_place,
                               departure_date, departure_time,
                               arrival_date, arrival_time,
                               COALESCE(mode_transport, mode_transport),
                               COALESCE(cost,0)
                          FROM travel_fare
                         WHERE travel_id=%s
                        """,
                    ...
                    fare_rows = [{
                        ...
                        "departure_date": (rf[3].isoformat() if hasattr(rf[3], 'isoformat') else (str(rf[3]) if rf[3] is not None else "")),
                        "departure_time": (str(rf[4]) if rf[4] is not None else ""),
                        "arrival_date": (rf[5].isoformat() if hasattr(rf[5], 'isoformat') else (str(rf[5]) if rf[5] is not None else "")),
                        "arrival_time": (str(rf[6]) if rf[6] is not None else ""),
                        "mode_transport": rf[7],
                    ...
                    cur.execute(
                        """
                        SELECT localfare_id, fromplace, toplace, mode_transport, number_km, COALESCE(amount,0), uploaded_date
                          FROM local_fare
                         WHERE travel_id=%s
                        """,
                        [travel_id]
                    ...
                    local_rows = [{
                        ...
                        "date": (rl[6].isoformat() if hasattr(rl[6], 'isoformat') else (str(rl[6]) if rl[6] is not None else "")),
                        "file_url": f"/api/files/local_fare/{int(rl[0])}/",
                    } for rl in cur.fetchall()]

                    cur.execute(
                        """
                        SELECT hotel_acc_id, checkin_date, checkout_date, hotel_name, adress, COALESCE(total_amount,0)
                    ...
                    hotel_rows = [{
                        "id": int(rh[0]),
                        "checkin_date": (rh[1].isoformat() if hasattr(rh[1], 'isoformat') else (str(rh[1]) if rh[1] is not None else "")),
                        "checkout_date": (rh[2].isoformat() if hasattr(rh[2], 'isoformat') else (str(rh[2]) if rh[2] is not None else "")),
                    ...
                    food_rows = [{
                        "id": int(rf2[0]),
                        "from_date": (rf2[1].isoformat() if hasattr(rf2[1], 'isoformat') else (str(rf2[1]) if rf2[1] is not None else "")),
                        "to_date": (rf2[2].isoformat() if hasattr(rf2[2], 'isoformat') else (str(rf2[2]) if rf2[2] is not None else "")),
                    ...
                    misc_rows = [{
                        "id": int(rm[0]),
                        "date": (rm[1].isoformat() if hasattr(rm[1], 'isoformat') else (str(rm[1]) if rm[1] is not None else "")),
                        "particulars": rm[2], "amount": float(rm[3] or 0),
